...
TEST_CASE("spawned command should return true", "[pipeline]")
{
  ...
  std::unique_ptr<Pipeline> pipeline =
      std::make_unique<Pipeline>("D:\\WorkFiles\\http_server\\build\\processes\\Debug\\process_exit_after.exe");
  pipeline->start();
...
TEST_CASE("spawned command should return true once again", "[pipeline]")
{
  std::cout << "Started\n";
  std::unique_ptr<Pipeline> pipeline =
      std::make_unique<Pipeline>("D:\\WorkFiles\\http_server\\build\\processes\\Debug\\process_exit_properly.exe");
  pipeline->start();
  REQUIRE(pipeline->is_running());
  REQUIRE(pipeline->is_running());
  REQUIRE(pipeline->is_running());
...
TEST_CASE("spawned command should return true in 2sec", "[pipeline]")
{
  std::cout << "Started\n";
